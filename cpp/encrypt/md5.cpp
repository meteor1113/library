...
namespace encrypt
{
     ...
     * context.
     */
    void MD5::Update(const void* in, unsigned int length)
    {
        ...
        if ((_count[0] += ((unsigned int)length << 3))
    ...
    {
        unsigned int a = _state[0], b = _state[1], c = _state[2];
        unsigned int d = _state[3], x[16];
     ...
     * a multiple of 4.
     */
    void MD5::Encode(const unsigned int* input, unsigned char *output,
                     unsigned int length)
    {
     ...
     * a multiple of 4.
     */
    void MD5::Decode(const unsigned char* input, unsigned int* output,
                     unsigned int length)
    {
        for (unsigned int i = 0, j = 0; j < length; i++, j += 4)
        {
     ...
     */
    std::string MD5::BytesToHexString(const unsigned char* input,
                                      unsigned int length)
    {
        std::string str;
    ...
    {
        return MD5Data(data, length).substr(8, 16);
    }
    ...
    {
        return MD5File(file).substr(8, 16);
    }
