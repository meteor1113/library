...
namespace encrypt
{
    ...
    void CRC32::Update(const void* in, unsigned int uBufSize)
    {
        const unsigned char* pData = (const unsigned char*)in;
        for(unsigned long i = 0; i < uBufSize; i++)
        {
            _value = (_value >> 8) ^ crc32tab[(pData[i]) ^ (_value & 0x000000FF)];
        }
    }
